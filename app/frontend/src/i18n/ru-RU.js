export default {
    "common": {
        "enter": "Подтвердить",
        "cancel": "Отмена",
        "name": "Имя",
        "save": "Сохранить",
        "check": "Пожалуйста, выберите",
        "add": "Создать",
        "edit": "Редактировать",
        "delete": "Удалить",
        "deleteFinish": "Успешно удалено",
        "topicNotFound": "Топик не найден в текущем списке, очистите поиск и повторите",
        "noConnToExport": "Нет подключений для экспорта",
        "exportConnOk": "Экспортировано подключений: {count}",
        "importConnOk": "Добавлено {imported}, обновлено {updated}, пропущено {skipped}",
        "deleteOk": "Вы действительно хотите удалить?",
        "count": "Количество",
        "refresh": "Обновить",
        "read": "Читать",
        "config": "Настройки",
        "csv": "Экспортировать в CSV",
        "json": "Экспорт в JSON",
        "connecting": "Подключение...",
        "action": "Действие",
        "compress": "кампрэсія",
        "decompress": "Распакуйце"
    },
    "aside": {
        "cluster": "Кластер",
        "node": "Узел",
        "topic": "Тема",
        "producer": "Производитель",
        "consumer": "Потребитель",
        "group": "Группа потребителей",
        "monitor": "Мониторинг",
        "sr": "Schema Registry",
        "settings": "Настройки"
    },
    "conn": {
        "title": "Кластер",
        "click_to_connect": "Нажмите для подключения",
        "add": "Добавить кластер",
        "exportConn": "Экспорт",
        "importConn": "Импорт",
        "edit": "Редактировать соединение",
        "test": "Тестировать соединение",
        "add_link": "Добавить соединение",
        "please_add_name": "Пожалуйста, введите имя",
        "please_add_link": "Пожалуйста, введите адрес соединения",
        "input_name": "Пожалуйста, введите имя",
        "bootstrap_servers": "Адрес соединения",
        "tip": "Примечание: Убедитесь, что локальная среда может получить доступ к адресу advertised.listeners Kafka (особенно для разрешения доменов, даже если вы вводите IP, вам нужно настроить hosts локально)",
        "tls": "Использовать TLS",
        "skipTLSVerify": "Пропустить проверку TLS",
        "tls_cert_file": "Введите путь к PEM-сертификату",
        "tls_key_file": "Введите путь к приватному ключу",
        "tls_ca_file": "Введите путь к CA-сертификату",
        "use_sasl": "Использовать SASL",
        "sasl_mechanism": "Механизм SASL",
        "sasl_user": "Имя пользователя SASL",
        "sasl_pwd": "Пароль SASL",
        "kerberos_user_keytab": "Путь к файлу keytab Kerberos",
        "kerberos_krb5_conf": "Путь к файлу krb5.conf Kerberos",
        "Kerberos_user": "Имя пользователя Kerberos",
        "Kerberos_realm": "Домен Kerberos",
        "kerberos_service_name": "Имя сервиса Kerberos",
        "oauthTip": "OAUTHBEARER: вставьте статический токен в поле пароля SASL",
        "oauthTokenPlaceholder": "Пароль SASL (используется как OAuth токен)",
        "sessionToken": "AWS Session Token",
        "sessionTokenOptional": "Необязательно: AWS session token",
        "mskTip": "AWS MSK IAM: AccessKey в поле пользователя, SecretKey в поле пароля. Требуется TLS",
        "use_ssh": "SSH-туннель",
        "ssh_host": "SSH-хост",
        "ssh_port": "SSH-порт",
        "ssh_user": "SSH-пользователь",
        "ssh_password": "SSH-пароль",
        "ssh_key_file": "Файл приватного ключа SSH"
    },
    "node": {
        "title": "Узел",
        "source": "Источник",
        "value": "Значение (двойной щелчок для редактирования)",
        "sensitive": "Чувствительно ли это?",
        "ok_message": "Редактирование успешно, обновление конфигурации",
        "controller": "Контроллер",
        "logdirs": "Директории логов",
        "logdirsTip": "Использование диска по директориям логов брокеров.",
        "logdirsPartitions": "Размеры партиций (топ-200)",
        "quotas": "Квоты",
        "addQuota": "Добавить квоту",
        "quotaEntityType": "Тип сущности",
        "quotaEntityName": "Имя сущности",
        "quotaEntityNameTip": "Пусто — сущность по умолчанию",
        "quotaKey": "Ключ квоты",
        "quotaValue": "Значение",
        "size": "Размер"
    },
    "topic": {
        "title": "Тема",
        "add": "Создать тему",
        "add_name": "Введите имя темы",
        "selectedGroup": "Выберите группу и прочитайте смещения",
        "partition": "Раздел",
        "add_partition": "Добавить раздел",
        "add_partition_count": "Количество добавляемых дополнительных разделов",
        "replication_factor": "Коэффициент репликации",
        "err": "Ошибка темы",
        "lag": "Опоздание",
        "viewProduce": "Производить сообщения",
        "viewConsumer": "Просмотреть/потреблять сообщения",
        "viewOffset": "Прочитать смещение",
        "viewConfig": "Настройки темы",
        "viewPartition": "Просмотр разделов",
        "deleteTopic": "Удалить тему",
        "deleteRecords": "Удалить записи",
        "deleteRecordsScope": "Область",
        "deleteRecordsAll": "Удалить все сообщения",
        "deleteRecordsBefore": "Удалить до offset",
        "deleteRecordsOffset": "Offset (не включая)",
        "deleteRecordsWarning": "ВНИМАНИЕ: удалённые записи не восстановимы!",
        "deleteRecordsDone": "Записи удалены",
        "reassign": "Переназначить реплики",
        "reassignJson": "Партиция → реплики JSON",
        "reassignTip": "Формат {\"0\":[1,2]}. Пустой массив/null отменяет текущее переназначение.",
        "reassignDone": "Переназначение отправлено",
        "reassignInvalidJson": "Неверный JSON"
    },
    "producer": {
        "title": "Производитель",
        "desc": "Клиент-производитель, который отправляет сообщения в указанную тему.",
        "selectTopic": "Выбрать тему",
        "topicPlaceholder": "Обязательно: Выберите или найдите Kafka тему",
        "optionalMessageKey": "Необязательно: Введите ключ сообщения",
        "keyPlaceholder": "Необязательно: Введите ключ сообщения",
        "specifyPartition": "Необязательно: Укажите номер раздела",
        "messageContentPlaceholder": "Обязательно: Содержание сообщения, строковый формат, поддерживается JSON",
        "headersTitle": "Заголовки сообщения:",
        "addHeader": "Добавить заголовок",
        "removeHeader": "Удалить",
        "headerKeyPlaceholder": "Ключ заголовка",
        "headerValuePlaceholder": "Значение заголовка",
        "sendTimes": "Количество отправок",
        "sendTimesPlaceholder": "Количество отправок",
        "sendMessage": "Отправить сообщение"
    },
    "consumer": {
        "title": "Потребитель",
        "desc": "Простой клиент-потребитель для просмотра сообщений из темы.",
        "requiredTopic": "Обязательно: Тема",
        "topicPlaceholder": "Выберите или найдите Kafka тему",
        "requiredMessagesCount": "Обязательно: Количество сообщений для потребления",
        "messagesCountPlaceholder": "Количество сообщений для потребления",
        "pollTimeoutDescription": "Время ожидания опроса: по умолчанию 10 секунд. Если есть исключения или нет сообщений для потребления, произойдет тайм-аут",
        "pollTimeoutPlaceholder": "Время ожидания опроса",
        "optionalGroup": "Необязательно: Группа (При выборе, смещения будут автоматически отправлены при потреблении. Поддерживает создание новых групп)",
        "groupPlaceholder": "Выберите или создайте группу потребителей",
        "noGroupPlaceholder": "Без группы (прямое чтение)",
        "noGroupOption": "Без группы (Direct Consume)",
        "noGroup": "Без группы",
        "groupTooltip": "Оставьте пустым для прямого чтения: группа потребителей не регистрируется, ребалансировка не вызывается, кластер не засоряется. Укажите группу для отслеживания прогресса и фиксации смещений.",
        "commitOffsetTooltip": "Закоммитить смещение после потребления",
        "commitOffsetDisabledTip": "Фиксация смещений невозможна без группы потребителей. Сначала укажите группу.",
        "firstConsumeTip": "Первое потребление может занять некоторое время для балансировки смещений",
        "consumeMessage": "Потребовать сообщения",
        "subscribe": "Подписаться",
        "stop": "Стоп",
        "live": "В эфире",
        "stoppedStatus": "Остановлен",
        "received": "Получено:",
        "msgPerSec": "сообщ/с",
        "autoScroll": "Автопрокрутка",
        "maxCache": "Макс:",
        "cacheTooltip": "Макс. количество: {count} | Макс. память: {size}",
        "startOffset": "Начальный Offset",
        "startOffsetTip": "Потреблять с этого offset на каждой партиции (приоритетнее позиции/времени)",
        "localSearch": "локальный поиск",
        "regex": "регулярка",
        "keyword": "ключевое слово",
        "filterModeTip": "Переключение между фильтром по слову и регулярным выражением",
        "showAdvanced": "Показать расширенные опции",
        "hideAdvanced": "Скрыть расширенные опции",
        "stopAll": "Остановить все",
        "replay": "Повторная отправка",
        "replayTip": "Отправить {count} сообщений (как в таблице) в целевой топик",
        "replayTarget": "Целевой топик",
        "replayEmpty": "Нет сообщений для отправки",
        "replayDone": "Отправлено {count} сообщений",
        "isLatest": "Потреблять с конца",
        "onlyTip": "Действительно только для разового потребления",
        "startTimestamp": "Начальная временная метка",
        "isolationLevel": "Уровень изоляции",
        "isolationLevelReadUncommitted": "Чтение незафиксированных (Read Uncommitted)",
        "isolationLevelReadCommitted": "Только зафиксированные (Read Committed)"
    },
    "group": {
        "title": "Группа потребителей",
        "member": "Участники",
        "warn": "Пожалуйста, переключитесь с конкретной темы на эту страницу",
        "resetOffset": "Сбросить Offset",
        "resetTitle": "Сброс offset группы потребителей",
        "resetStrategy": "Стратегия",
        "resetToEnd": "К последним",
        "resetToStart": "К первым",
        "resetToTimestamp": "К временной метке",
        "resetToAbsolute": "К указанному offset",
        "resetOffsetValue": "Offset",
        "resetTopics": "Топики (пусто = все закоммиченные)",
        "resetTip": "В группе не должно быть активных участников. Приложение сначала отключит своих потребителей.",
        "resetDone": "Сброшено партиций: {count}",
        "needTimestamp": "Выберите временную метку"
    },
    "inspection": {
        "title": "Мониторинг",
        "desc": "Проверка состояния задержек в Kafka.",
        "topicsLabel": "Темы",
        "topicPlaceholder": "Выберите или найдите Kafka тему",
        "groupLabel": "Группа",
        "groupPlaceholder": "Выберите или создайте группу потребителей",
        "startInspection": "Начать мониторинг",
        "autoFetch": "Автоматическое получение данных каждые 5 минут",
        "lagFormula": "Задержка = Конечное смещение - Закоммиченное смещение.",
        "lag": "Задержка",
        "commit": "Здзейснены зрух",
        "end": "Канцавое зрушэнне",
        "schemeNamePlaceholder": "Введите название схемы",
        "saveScheme": "Сохранить схему",
        "loadSchemePlaceholder": "Выберите схему мониторинга",
        "deleteScheme": "Удалить схему",
        "deleteSchemeConfirm": "Подтверждение удаления",
        "deleteSchemeConfirmContent": "Вы уверены, что хотите удалить схему '{name}'? Это действие необратимо.",
        "stopInspection": "Остановить инспекцию",
        "alertConfig": "Настройка оповещений",
        "enableAlert": "Включить оповещения",
        "webhookUrl": "URL вебхука",
        "webhookUrlPlaceholder": "Введите URL вебхука для оповещений",
        "customHeader": "Пользовательский заголовок",
        "customHeaderPlaceholder": "Например: Authorization: Bearer xxxxx",
        "threshold": "Порог отставания",
        "thresholdPlaceholder": "Введите порог отставания",
        "messageTemplate": "Шаблон сообщения",
        "messageTemplatePlaceholder": "Опционально. Поддерживаемые плейсхолдеры: [total_lag], [topics], [group], [timestamp]",
        "checkInterval": "Интервал проверки",
        "checkIntervalPlaceholder": "Интервал проверки (в минутах)"
    },
    "settings": {
        "title": "Настройки",
        "width": "Ширина окна",
        "height": "Высота окна",
        "lang": "Язык",
        "theme": "Тема"
    },
    "message": {
        "noMemberFound": "Участников не найдено",
        "saveSuccess": "Успешно сохранено",
        "connectSuccess": "Успешно подключено",
        "fetchSuccess": "Успешно получено",
        "sendSuccess": "Успешно отправлено",
        "selectGroupFirst": "Пожалуйста, сначала выберите группу",
        "selectTopic": "Пожалуйста, выберите тему",
        "selectTopicGroup": "Пожалуйста, выберите тему и группу",
        "connectErr": "Ошибка подключения",
        "addOk": "Успешно добавлено",
        "editOk": "Успешно отредактировано, обновление конфигурации",
        "mustFill": "Пожалуйста, заполните все обязательные поля",
        "saveErr": "Ошибка сохранения",
        "pleaseInput": "Пожалуйста, введите содержание сообщения",
        "streamStarted": "Подписка запущена",
        "schemeNameRequired": "Пожалуйста, выберите схему",
        "schemeSaved": "Схема успешно сохранена",
        "schemeDeleted": "Схема успешно удалена",
        "schemeLoaded": "Схема загружена: {name}",
        "alertSent": "Оповещение отправлено",
        "alertSendFailed": "Ошибка отправки оповещения"
    },
    "about": {
        "title": "О программе",
        "projectHomepage": "Домашняя страница проекта",
        "kafkaKing": "Kafka-King",
        "esClient": "Сопутствующий ES клиент",
        "esKing": "ES-King",
        "newProject": "Рекомендуемый проект",
        "allyAgent": "Ally Agent",
        "allyAgentTag": "ИИ-ассистент",
        "allyAgentDesc": "Опенсорсный десктопный ИИ-ассистент для написания кода и работы с локальными проектами. Помогает изучать код, редактировать файлы и управлять задачами через диалог.",
        "technicalGroup": "Группа технического обмена",
        "qqGroup": "QQ группа обмена",
        "translate": "Ці ёсць праблемы з перакладам? Паведаміць або прыняць удзел у перакладзе"
    },
    "header": {
        "desc": "Более удобный интерфейс для работы с Kafka",
        "c_node": "Текущий кластер",
        "netErr": "Не удается подключиться к GitHub, проверьте сетевое соединение",
        "newVersion": "Доступна новая версия",
        "down": "Скачать сейчас"
    },
    "sr": {
        "title": "Schema Registry",
        "connected": "Подключено",
        "notConnected": "Не подключено",
        "connect": "Подключить",
        "user": "Пользователь",
        "userOptional": "Необязательно",
        "password": "Пароль",
        "passwordOptional": "Необязательно",
        "versions": "Версии",
        "selectVersion": "Выбрать версию",
        "deleteVersion": "Удалить версию",
        "compatibility": "Совместимость",
        "setCompatibility": "Задать совместимость",
        "cycleCompatibility": "Задать совместимость",
        "urlRequired": "Введите URL"
    },
    "acl": {
        "title": "Управление ACL",
        "add": "Добавить ACL",
        "edit": "Редактировать ACL",
        "delete": "Удалить ACL",
        "principal": "Принципал",
        "principalPlaceholder": "Введите принципал, например User:alice",
        "resourceType": "Тип ресурса",
        "resourceTypePlaceholder": "Выберите тип ресурса",
        "resourceName": "Имя ресурса",
        "resourceNamePlaceholder": "Введите имя ресурса",
        "operation": "Операция",
        "operationPlaceholder": "Выберите операцию",
        "permissionType": "Тип разрешения",
        "permissionTypePlaceholder": "Выберите тип разрешения"
    }
};
